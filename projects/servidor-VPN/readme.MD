...
Instalamos OpenVPN directamente desde los repositorios oficiales de Debian junto con `git` para clonar Easy-RSA:
...
### Firmar el certificado del cliente (profesor) y crear la clave TLS

```bash
./easyrsa build-client-full profesor-mgarfer1604 nopass
```

Tras firmar el certificado del profesor, generamos la clave `ta.key` para protección adicional contra ataques DoS (TLS Authentication):
...
> **Nota:** En versiones recientes de OpenVPN el flag `--secret` está deprecado. Si aparece el aviso, usar `--genkey secret <filename>` es igualmente válido.
...
Creamos tres directorios para organizar los certificados por rol y copiamos los ficheros correspondientes:
...
Genera el archivo `pki/dh.pem` con parámetros DH de 2048 bits. Se copia a la carpeta del servidor:
...
Se añade la línea `dh` al `server.conf` apuntando al fichero generado:
...
El servidor inicializa correctamente: carga los parámetros DH, levanta la interfaz `tun0` y queda a la escucha en el puerto UDP 11218:
...
La interfaz `tun0` aparece con la IP `192.168.218.1/32` y el peer `192.168.218.2`:

![img18](images/img18.jpg)

### Conectar el cliente y verificar

Desde la máquina cliente, primero comprobamos conectividad básica con ping al servidor:

![img19](images/img19.jpg)

Transferimos los archivos necesarios al cliente vía SCP:
...
Lanzamos el cliente OpenVPN:

```bash
sudo /usr/sbin/openvpn --config cliente.ovpn
```

El cliente negocia TLS correctamente con el servidor, obtiene la IP `192.168.218.6` en la interfaz `tun0` y el ping al servidor VPN (`192.168.218.1`) responde sin pérdida de paquetes:
